 ...
 * @constructor
 */
function DelegateAlertView (config) {
...
extendFunction(DelegateAlertView, View);

DelegateAlertView.prototype.onStarted = function () {
 ...
 * @param {string} error
 */
DelegateAlertView.prototype.showError = function (error) {
    this.errorElement.innerHTML = __("metapayalertview.error." + error);
    this.errorElement.show();
};

DelegateAlertView.prototype.hideError = function () {
    this.errorElement.hide();
};

DelegateAlertView.prototype.onWalletsLoaded = function () {
    let wallets = this.app.walletCollection.getWalletsSortedByTime(this.currencyId);
    if (wallets.length) {
        this.hideError();
...
DelegateAlertView.prototype.onWalletSelect = function () {
    if (!this.app.walletCollection.getWalletsSortedByTime(this.currencyId).length) {
        return;
    }

    this.app.alertView(new WalletSelectAlertView({app: this.app, currencyId: this.currencyId, selectedAddress: this.wallet ? this.wallet.address : ""}))
 ...
 * @param {Wallet} wallet
 */
DelegateAlertView.prototype.onWalletSelected = function (wallet) {
    this.wallet = wallet;
...
DelegateAlertView.prototype.onContinueClicked = function () {
    ...
    this._resolve({wallet: this.wallet, metaPayParams: this.delegateParams});
};