 ...
 * @constructor
 */
function MetaPayAlertView (config) {
    ...
    /** @type {Currency} */
    this.currency = new Currency({walletCollection: this.app.walletCollection, currency: this.metaPayParams.currencyId});

    /** @type {?Wallet} */
    this.wallet = null;
}

extendFunction(MetaPayAlertView, View);

MetaPayAlertView.prototype.onStarted = function () {
 ...
 * @param {string} error
 */
MetaPayAlertView.prototype.showError = function (error) {
    this.errorElement.innerHTML = __("metapayalertview.error." + error);
    this.errorElement.show();
};

MetaPayAlertView.prototype.hideError = function () {
    this.errorElement.hide();
};

MetaPayAlertView.prototype.onWalletsLoaded = function () {
...
MetaPayAlertView.prototype.onWalletSelect = function () {
    if (!this.app.walletCollection.getWalletsSortedByTime(this.metaPayParams.currencyId).length) {
        return;
    }

    this.app.alertView(new WalletSelectAlertView({app: this.app, currencyId: this.metaPayParams.currencyId, selectedAddress: this.wallet ? this.wallet.address : ""}))
 ...
 * @param {Wallet} wallet
 */
MetaPayAlertView.prototype.onWalletSelected = function (wallet) {
    this.wallet = wallet;
...
MetaPayAlertView.prototype.onContinueClicked = function () {
    ...
    this._resolve({wallet: this.wallet, metaPayParams: this.metaPayParams});
};