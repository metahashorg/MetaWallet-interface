...
extendFunction(TransferView, View);

TransferView.prototype.onStarted = function () {
...
TransferView.prototype.onScanQrCodeClicked = function () {
    bridgeCallHandler("openQRscan")
        .then(function (data) {
            ...
            if (BlockchainLib.checkHashTypeForCurrency(hashType, this.wallet.currencyId)) {
                ...
                this.app.showNotification({text: __("importpk.wrongcurrency", {currency: CURRENCY_CODES[this.wallet.currencyId]}), type: NOTIFICATION_ERROR, hideTimeout: UI_ERROR_NOTIFICATION_LONG_TIMEOUT * 1000});
                this.formElement.querySelector("[name='to']").value = "";
            } else {
                this.app.showNotification({text: __("error.wrongqrcode"), type: NOTIFICATION_ERROR, hideTimeout: UI_ERROR_NOTIFICATION_LONG_TIMEOUT * 1000});
                this.formElement.querySelector("[name='to']").value = "";
            }
        }.bind(this))
...
TransferView.prototype.onSendClicked = function () {
    if (this.loader) {
        return;
    }
    ...
    if (this.wallet.balance < formData.value || !formData.value) {
        ...
        if (formData.value) {
            this.app.showNotification({text: __("error.notenoughfunds"), type: NOTIFICATION_ERROR, hideTimeout: UI_ERROR_NOTIFICATION_LONG_TIMEOUT * 1000});
        } else {
 ...
 */
TransferView.prototype.onTxSent = function (formData, result) {
    /** @type {Transfer} */
 ...
 */
TransferView.prototype.onTxError = function (result) {
    result = result || {errorCode: "notaccepted"};

    this.app.showNotification({text: __("transferview.error." + result.errorCode), type: NOTIFICATION_ERROR, hideTimeout: UI_ERROR_NOTIFICATION_LONG_TIMEOUT * 1000});
};